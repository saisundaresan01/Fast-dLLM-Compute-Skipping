...
def Fast_dLLM_QwenModel_forward(
    ...
    block_past_key_values: Optional[Cache] = None,
    replace_position: Optional[int] = None,
    skip_policy: Optional[LayerSkipPolicy] = None,
    ...
    **kwargs
    ...
    batch_seq = hidden_states.shape[0] * seq_len
    for decoder_layer in self.layers[: self.config.num_hidden_layers]:
        if skip_policy:
            skip_policy.update_hidden_states(hidden_states)
            if skip_policy.check_layer_skip():
                if compute_counter:
                    compute_counter.log_layer_skip(batch_seq)
                continue
        if compute_counter:
            compute_counter.log_tokens(batch_seq)
                ...
                while True:
                    ...
                    if use_block_cache:
                        if block_past_key_values is None or (x_t[:, -block_size+small_block_start_idx] == mask_id).any():
                            ...
                            # cached sub-block forward — pass policy for layer skipping
                            output = self.forward(
