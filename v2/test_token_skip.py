...
fix_seed(42)
model.generate(model_inputs["input_ids"], skip_policy=TokenSkipPolicy(threshold=1.0), compute_counter=baseline_counter, **gen_kwargs)
baseline_processed_tokens = baseline_counter.processed_tokens

for thresh in [0.995, 0.99, 0.98, 0.95]:
    print(f"\n=== TOKEN SKIP (threshold={thresh}) ===")
    policy = TokenSkipPolicy(threshold=thresh)
